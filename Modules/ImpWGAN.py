...
class ImpWGAN(object):
    ...
    def train(self, dataset, batch_size, n_iters, dis_iters_per_gen_iter, lmbda, optimizer_details,
        ...
        #    then fakes and then evaluate the gradient penalty term also
        ...
        while not flag:
            ...
            while i < len(d_loader):
                ...
                while j < dis_iters_per_gen_iter and i < len(d_loader):
                    ...
                    X, _ = cur_data
                    X = X.view(-1, self.image_size*self.image_size*self.n_chan)
                    if inpt.size() != X.size():
                        inpt.resize_(X.size(0), X.size(1))
                    inpt.copy_(X)
                    inptV = V(inpt)
                    ...
                    epsilon = t.rand(X.size(0), 1)
                    epsilon = epsilon.expand_as(X_f)
                    diffs = X_f.cpu().data - X
                    intrplts = X + epsilon*diffs
                    if self.ngpu > 0:
                        intrplts = intrplts.cuda()
                    intrpltsV = V(intrplts, requires_grad=True)
                    grads = t.autograd.grad(outputs=self.Dis_net(intrpltsV).mean(),
                                            inputs=intrpltsV, create_graph=True, retain_graph=True,
                                            only_inputs=True)[0]
                    grad_pen = (((grads.norm(2, dim=1) - 1).pow(2)).mean()) * lmbda
                    grad_pen.backward(pos)

                    err_D = err_D_f - err_D_r + grad_pen
                    D_optmzr.step()
                    j = j + 1
                ...
                if display_images:
                    if gen_iters % (show_period * 5) == 0:
                        ...
                        tv_utils.save_image(gen_imgs.data,
                                            'ImpWGAN_Generated_images@iteration={0}.png'.format(gen_iters))
...
class Generator(nn.Module):
    def __init__(self, image_size, n_z, n_chan, n_hidden, ngpu):
        ...
        main_fc.add_module('linear_{0}-{1}-{2}'.format(layer, n_z, 4 * 4 * n_hidden),
                           nn.Linear(n_z, 4 * 4 * n_hidden))
        ...
        while cur_size < image_size // 2:
            layer = layer + 1
            main_conv.add_module('conv_{0}-{1}-{2}'.format(layer, n_hidden, n_hidden // 2),
                                 nn.ConvTranspose2d(n_hidden, n_hidden // 2, kernel_size=4, stride=2, padding=1))
        ...
        layer = layer + 1
        main_conv.add_module('conv_{0}-{1}-{2}'.format(layer, n_hidden, n_chan),
                             nn.ConvTranspose2d(n_hidden, n_chan, kernel_size=4, stride=2, padding=1))
        if n_chan > 1:
            main_conv.add_module('TanH_{0}'.format(layer), nn.Tanh())
        else:
            main_conv.add_module('Sigmoid_{0}'.format(layer), nn.Sigmoid())
    ...
    def forward(self, input):
        ...
        pass_ = pass_.view(-1, self.image_size * self.image_size * self.n_chan)
        return pass_
...
class Discriminator(nn.Module):
    def __init__(self, image_size, n_chan, n_hidden, ngpu):
        ...
        # Details to be followed:
        # 1. ReLU activations everywhere
        # 2. No BatchNorm anywhere
        # 3. Last Layer is a simple fully-connected layer

        main_conv.add_module('conv_{0}-{1}-{2}'.format(layer, n_chan, n_hidden),
                             nn.Conv2d(n_chan, n_hidden, kernel_size=4, stride=2, padding=1))
        ...
        while cur_size > 4:
            layer = layer + 1
            main_conv.add_module('conv_{0}-{1}-{2}'.format(layer, n_hidden, n_hidden * 2),
                                 nn.Conv2d(n_hidden, n_hidden * 2, kernel_size=4, stride=2, padding=1))
            main_conv.add_module('LeakyReLU_{0}'.format(layer), nn.LeakyReLU(0.2, inplace=True))
            cur_size = cur_size // 2
            n_hidden = n_hidden * 2
    ...
    def forward(self, input):
        pass_ = input.view(-1, self.n_chan, self.image_size, self.image_size)
        if self.ngpu > 1:
            pass_ = nn.parallel.data_parallel(self.main_conv, pass_, range(0, self.ngpu))
            pass_.view(-1, 4 * 4 * self.n_hidden)
            pass_ = nn.parallel.data_parallel(self.main_fc, pass_, range(0, self.ngpu))
        else:
            pass_ = self.main_conv(pass_)
