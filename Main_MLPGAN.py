...
parser.add_argument('--n_gen_hidden', type=int, default=512, help='input number of feature maps in the first layer of generator')
parser.add_argument('--n_dis_hidden', type=int, default=512, help='input number of feature maps in the first layer of discriminator')
parser.add_argument('--gen_depth', type=int, default=3, help='input the depth of the generator network')
parser.add_argument('--dis_depth', type=int, default=3, help='input the depth of the discriminator network')
...
G_depth		= opt.gen_depth
D_depth		= opt.dis_depth
n_chan		= opt.n_chan
...
G_net		= mlp.Generator(img_size, n_z, n_chan, n_G_hidden, G_depth, ngpu)
D_net		= mlp.Discriminator(img_size, n_chan, n_D_hidden, D_depth, ngpu)
...
for epoch in range(0, opt.max_epochs):
	for i, itr in enumerate(d_loader):
		...
		noise.copy_(t.randn(b_size, n_z))
		noisev	= V(noise)
		fake	= G_net(noisev)
		label.fill_(0)
		labels	= V(label)
